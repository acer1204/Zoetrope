...
- **HDR 正確顯示**：EXR / Radiance HDR / JXR 保留浮點資料，並依來源選擇正確的
  預設值。**顯示參考**的 scRGB 截圖（Windows/NVIDIA 的 HDR 截圖）會把內容的
  「紙白」拉回 SDR 白（約 −1.5 EV）——這組數值是實測 Windows 相簿的畫面反推
  校正的，因此亮度與相簿一致；**場景參考**的 EXR 則套 ACES 保留高光滾降。
  另提供即時曝光補償（±6 EV，拉滑桿約 5ms，不需重新解碼）可查看高光細節
